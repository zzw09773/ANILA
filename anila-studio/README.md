...
> 從 `myCSPPlatform/backend` 抽出的**簡報生成服務**：RAG 找你上傳過的文件 → LLM 生大綱 → FLUX 生插圖 → PPTX 渲染。

> 中文為主版 · [English version](README.en.md)

> 🌿 **分支對照**：本服務存在於 `main` / `prod-intranet-card` / `prod-public-passwd` / `prod-military-passwd` / `dev-public` / `dev-military`。**`trial-military` 精簡版不含本服務**（移除 Studio / FLUX 簡報生成）。分支策略見根目錄 [`README.md`](../README.md) 的分支對照表與 [`docs/branch-sync-backlog.md`](../docs/branch-sync-backlog.md)。
...
抽出歷史與決策見 [`docs/superpowers/anila-studio/extraction-decision.md`](../docs/superpowers/anila-studio/extraction-decision.md)（2026-05-23 / PR #12）。
...
├── openapi/studio.openapi.json  # codegen 給 ANILALM 用的 contract
├── app/
...
│   ├── api/studio.py       # /api/studio/slides/jobs（4 endpoint）
│   ├── clients/csp_client.py    # 5 個 thin async function 包 csp HTTP API
│   ├── services/
...
│   └── schemas/studio.py   # pydantic SlidesSpec / JobStatus / GenerateSpecRequest 等
...
.venv/bin/pytest                                            # 跑 test（不需 docker）
...
Redis 失聯時 anila-studio **fail-closed**：`/health` 503 + 所有需 auth 的 endpoint 503，不允許「降到 TTL-only」（plan v2 R14）。

---

## Frontend（ANILALM）如何呼叫

`ANILALM/src/api/studio.ts` 透過 `VITE_STUDIO_BASE_URL` 指向 anila-studio（留空 → vite dev proxy 或 nginx reverse-proxy）。TypeScript types 從 `anila-studio/openapi/studio.openapi.json` codegen：

```bash
cd anila-studio && .venv/bin/python scripts/export-openapi.py   # 改了 schema 後重 gen
...
| `JWT_KID` / `JWT_ALGORITHMS` | `anila-v1` / `("RS256",)` | JWT kid + 接受的簽算法 |
...
- csp 端 `JWT_PRIVATE_KEY_PATH` 必須存在（`scripts/generate-jwt-keypair.py` 預生或 Vault 注入）；anila-studio 不需 private key，只需 csp `/.well-known/jwks.json` 可達。
- anila-studio 啟動會 fail-fast 若 csp `/api/auth/revocations` 不可達 — 確保 `depends_on: csp` 與 health gate 對齊。
...
- 抽出計畫：`docs/superpowers/anila-studio/plans/2026-05-23-extraction-plan.md`（plan v2）
