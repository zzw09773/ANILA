...
def _datetime_field_paths(
    model: type, prefix: str = "", *, _seen: frozenset[type] | None = None
) -> list[str]:
    """Return dotted paths of datetime-typed fields, including nested models.

    ``_seen`` breaks cycles (e.g. ``DepartmentTreeNode.children``).
    """
    ...
    for name, field in model.model_fields.items():
        ...
        for nested in _unwrap_response_model(ann):
            paths.extend(_datetime_field_paths(nested, path, _seen=seen))
        # list[NestedModel]
        ...
        if origin in (list, tuple):
            for arg in get_args(ann):
                for nested in _unwrap_response_model(arg):
                    paths.extend(
                        _datetime_field_paths(nested, f"{path}[]", _seen=seen)
                    )
    return paths
