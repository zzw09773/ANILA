...
#
# Sprint 8 X / Phase D — auto-bootstrap on first start.
#
# Behaviour:
...
#      ``anila-core agent bootstrap`` to exchange the bsk- for a
#      csk- and write the state file. Required env: $CSP_URL,
...
if [ -f "${STATE_FILE}" ]; then
    ...
    echo "[entrypoint] running anila-core agent bootstrap (csp=${CSP_URL}, agent_id=${ANILA_AGENT_ID})"
