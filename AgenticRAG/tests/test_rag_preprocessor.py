...
from anila_core.engine.rag_preprocessor import (
    RagPreprocessor,
...
def make_chunk(content: str, score: float = 0.9, source: str = "test.pdf") -> DocumentChunk:
    return DocumentChunk(
        ...
        content=content,
        embedding=[0.1] * 4096,
        metadata={"score": score, "source_path": source},
    )
...
class MockRetriever:
    ...
    async def search(self, query_embedding, user_id, project_id, top_k=5, min_score=0.0):
        return [c for c in self._chunks if c.metadata.get("score", 0) >= min_score][:top_k]
