"""Bootstrap CLI — exchange a bsk- for a long-lived csk- token.

Used once on agent first start (or after admin rotation) to populate
the per-agent service-token state file that
``RotatingServiceTokenMiddleware`` reads.

Wire contract: see ``docs/csp-agent-bootstrap-protocol.md`` (frozen
in Phase 0.5). Identical to the contract used by anila-core's CLI;
the two implementations must stay in sync because both call the same
CSP endpoint with the same payload shape.
...
* Writes the returned csk- + metadata to
  ...
  on Windows — chmod is a no-op there).
...
def run(args: list[str]) -> None:
    ...
    parser keeps the door open for ``rotate`` / ``show`` later
    without breaking the CLI surface.
    """
    parser = argparse.ArgumentParser(
        prog="python -m agentic_rag.cli bootstrap",
        description=(
            "Exchange an admin-issued bsk- for a long-lived csk- service "
            "token and persist it to the agent state file."
        ),
    )
    parser.add_argument(
        "--csp-url",
...
def _write_state_file(
    *,
    ...
    # bits but doesn't error; the volume mount usually delivers
