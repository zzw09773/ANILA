...
| `myCSPPlatform/docker/Dockerfile` | `infra/docker/csp.Dockerfile`（**唯一**） | 正式 CSP image 來源；compose 用 root build context 並 COPY `anila-core`、CSP backend、CSP frontend build。`myCSPPlatform/backend/Dockerfile` 不應當部署來源。⚠ 本欄原本還列了 `services/csp/Dockerfile` 當替代位置——那份確實被建過、但 compose 從不用它，2026-08-06 已刪除（FAKE-CONTROLS #50：「修正不在部署路徑上」）。**不要再建第二份 CSP Dockerfile。** |
| `myCSPPlatform/docker/nginx.conf` | `infra/nginx/anila.conf` | 可移植為唯一外部入口設定。現況含 `443` CSP / `/api` / `/v1` / `/router` / `/anila/`（同源 ANILA runtime UI 主要入口，見 doc 02 §10）/ `/anilalm` / `/n8n` / `/codeserver`。`/gitlab` 已於 2026-09-26 移除。`4443` 為 runtime UI 的 legacy / 替代入口，Studio artifact regex route 到 `anila-studio:8100`，且有 rate limit / security headers。 |
