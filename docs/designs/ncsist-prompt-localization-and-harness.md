...
2. **全平台沒有任何一處寫國家語境**：gpt-oss-20b／gemma4 的訓練語料大陸占比高，兩岸相關內容有吐出「中国台湾」「台湾省」這類 PRC 官方框架用語的真實風險——對本院使用者這是嚴重錯誤。也**完全沒有民國紀年規則**（中科院院內文件滿是民國紀年，模型會把「114 年」當 2014 或 114 西元年）。
...
7. **民國紀年三件套**：共同前導規則（§3）＋ingestion 正規化（索引時「114年」同時以「2025」入索引欄位）＋查詢改寫（使用者查「113年度測評」自動擴展西元）。中科院院內文件的年度檢索正確率直接受益。
