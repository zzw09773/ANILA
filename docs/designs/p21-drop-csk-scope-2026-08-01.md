...
## 六、唯一待決的設計決定（開工時兩案一頁請擁有者選）

**任務外呼叫**（runtime-config 輪詢、revocation feed）沒有派工憑條可用：
- a) agent 註冊時登記公鑰、自簽短效 assertion 換 token（標準 private_key_jwt；零長效祕密，多一段 PKI 管理）；
- b) 留一顆**低權限 poll-only** 長效小鑰匙（只能拉設定／撤銷清單，不能冒充使用者、不能搜資料；
  簡單，`.env` 留一行）；
- c) 拿掉輪詢改推送。
