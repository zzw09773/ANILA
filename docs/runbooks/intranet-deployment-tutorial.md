...
| # | 項目 | 為什麼它在這裡 |
|---|---|---|
| 1 | **在出貨映像上實測破窗**（`/login?show_alternatives=1` 要真的出現帳密欄位） | 演練當時的映像上**失效過**。這是卡登壞掉時**唯一的回頭路**，而發現它壞掉的人，正好是那個進不去平台的人 |
| 2 | **F-1 的隱藏行為確認** | 同一族：登入頁不該提供這個部署用不了的門 |
| 3 | **確認包的 `Ref` 是乾淨 tag** | 見 §4.2 |
