...
⚠ 這條跟本專案的總綱同構（見 `docs/archive/handoffs/HANDOFF-2026-08-11.md` 開頭）：
**把「沒發生的事」變成「明說的事」。**
...
| `share/static/`（與其他被容器寫過的 `share/*`） | `intranet-deploy.sh` `[2/7]` 的可寫檢查 | **不是跳過，是中止**：目錄由容器以 root 建立後，主機帳號寫不進去，腳本印「share 目錄不可寫」停下。2026-09-02 演練實撞（`down -v` 不會動 bind mount） |

**處置：演練前把 `share/` 交回主機帳號**（無 sudo 時用容器）：
```bash
docker run --rm -v "$PWD/share:/s" alpine:3.20 sh -c "chown -R $(id -u):$(id -g) /s"
```

### 🟡 演練包會被腳本擋下——那是對的，要明說
`[4/7]` 會檢查 bundle 的 `Ref`：**不是乾淨 tag（例如 `v2.0.1-722-gbafb6577`）就拒絕載入**，提示
「這是演練包，不得部署到正式機」。**演練環境要設 `ALLOW_REHEARSAL_BUNDLE=1` 才過**；正式出貨包必須從打好的 tag 重建。
2026-09-02 演練：第一次沒設就停在這裡——**這一停是守衛在做事，不是缺陷**。
