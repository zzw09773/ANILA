...
--
-- ⚠ 2026-07-26 更新:**這支不再是部署阻斷,只是健檢。** user 確認 `.15` 也是
-- 新建、院內尚未正式開放使用,所以「舊庫累積了一批髒資料」的前提不成立。
-- 原本掛在它後面的兩件事都已經直接做掉了:
--   - 孤兒欄清理 → `r1_0037_drop_orphan_fk_columns.py`(內含安全閘:欄位若有
--     任何非 NULL 列就 raise 並拒絕繼續,已用 seed 資料驗過)。
--   - alerts 去重成本 → r1_0036 本來就是「補齊缺失」的冪等姿態。
-- 還是建議在第一次正式部署前跑一次:成本是五分鐘,而它把假設換成數字。
--
-- 為什麼要在 .15 上跑這支
-- ------------------------
