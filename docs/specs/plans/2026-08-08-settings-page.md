...
> 規格權威＝`docs/specs/2026-08-08-settings-page-design.md`（含 08-08 三項追加裁決）。
...
1. **⚠ 命門：真值判定必須與消費端共用同一個函式。** `card_auth.py:121` 讀旗標**不 strip**
   ...
   突變「守衛換成自己的解析」必死。
   ⚠ **上面那個反向例子是寫反的，2026-08-09 原地更正**（Task 7 報告 §5 已對真碼查核）：
   消費端**不 strip**，所以「` true `」**不會**啟用消費端（`" true ".lower()` 不在
   `("1","true","yes")` 裡）——補空白反而是**關的**。真正會出人命的方向是
   **守衛比消費端窄**：守衛只認 `== "true"`，而 `CARD_DEV_SKIP_NONCE_BINDING=yes`
   會讓消費端啟用 → 守衛放行、旁路照開。「守衛比消費端寬」（自己補了 `strip()`）
   則是假警報：擋住開機，而消費端其實是關的。**兩個方向都是缺陷，所以共用同一支函式。**
...
3. `docs/specs/2026-08-08-settings-page-design.md`：與**實際出貨**對齊
