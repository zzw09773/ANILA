...
🟢 **實際出貨的數字（2026-08-09 收尾核對；登錄表 `settings_registry.py` 共 96 筆）**
...
Task 4 把開機覆蓋接上去之後，量到**七顆 B-可編輯是在覆蓋能生效之前就被消費掉的**
（逐顆有讀碼證據：`DEBUG`、`STATIC_DIR`、`ANILA_HOST`、`PYTHONUNBUFFERED`、
...
🟢 **這個機制照原句出貨了（2026-08-09 核對）**：`apply_boot_overrides()` 在 lifespan
裡（alembic 之後、任何消費端之前）**就地 `setattr`** 那顆 `Settings` 單例；
`.env` 一個字都沒寫、零新增掛載、零 alembic migration。載入失敗時
`BootOverrideSnapshot.load_failed` 為真，畫面最上面出現大字
**「這次開機沒有載入設定覆蓋」**——原句要求的「絕不假裝覆蓋生效了」有專屬測試釘著。
⚠ 原句沒預見的那一半：**有七顆 B 類在覆蓋能生效之前就被消費掉**，它們留在「可編輯」
就會變成假控制項，所以降成 B-鎖定（見上表的兩段式說明）。
...
🟢 **這兩件的實際出貨都與原句有出入，兩處都是原句錯**（2026-08-09）：

- **`FLUX_BACKEND_URL` 沒有被刪**：原句的「全樹零讀取點」是 **csp 範圍內**才成立的說法，
  它對 **anila-studio 是活的 toggle**（空值＝關）。所以 `.env.example` 與
  `platform.yml` 給 anila-studio 的那一行都留著，只有 csp 區塊改成歸檔註記。
  `LLM_MODEL` 同理留著（`dev.yml` 在用）。⚠ `dev.yml:121-123`／`:242` 的殘留是**明白揭露的
  範圍外**，待掃。
- **守衛的判準比原句嚴**：原句只說「設了此旗標→拒」，而消費端 `card_auth` 是在
  **import 當下把旗標凍結**成模組常數的。只讀環境的守衛會漏掉「以 `=1` import、
  開機前把變數移除」那個視窗（旁路凍結成開著、守衛看不到）——跨家審查實測到。
  出貨的守衛判 **frozen OR live，frozen 是主判準**。
...
- 🟢 **門檻的「校準畫面」缺口只關掉一半**（Q41 原句：「範本出處＝設計文件 §7，
  門檻設定＝第一個實例，後端半已驗證，**前端半本輪補齊**」）。
  實際出貨：`institutional_kb.score_threshold` 現在確實**可以從設定頁改**（C 類，改完
  下一個請求生效），但**校準要用的 `POST /api/institutional-kb/preview` 仍然沒有畫面**
  ——分數分布只能走 API 看。沿用既有寫法（`HANDOFF-2026-08-07.md` §七第 8 點）：
  **上線前要嘛補畫面、要嘛把「怎麼用 API 校準」寫進運維手冊**，
  而且要在內網量測清單被人拿去執行**之前**決定——那份清單不可以把操作者送到一個不存在的畫面。
...
- B 類 40 顆的「需重啟」標記措辭（「改 `.env` 後 `up -d` 該服務」）由 Task 實作時
  ...
  **`已儲存，重啟後生效：docker compose up -d --force-recreate csp；重開後回頭看這一列，「現在生效」要變成你存的值`**
  （原句寫「改 `.env` 後」，而 B-可編輯**根本不改 `.env`**——值在 DB，要的只是一次新的開機）。
  ⚠ **`--force-recreate` 不可省**（2026-08-09 更正）：值改在 DB，compose 檔與容器的設定雜湊
  一個字都沒變，裸 `up -d` 會判定「已是最新」而**根本不重建容器** —— 那是一道什麼也不做的
  命令，正是這一頁存在要消滅的形狀。**字面權威是**
  `apps/csp-governance-ui/src/utils/settingsView.js` 的 `RECREATE_COMMAND`／`RESTART_HINT`
  常數（本行只是轉錄，26822978 修頁面、a05b789b 修 runbook 之後補上）：要改請改那兩個常數，
  不要在文件裡另外釘一份字面 —— 這一行寫於 01508e9f，就是因為沒有回頭改而變成了
  「權威文件記載著一道 no-op 命令」。
  操作面連同「怎麼確認容器真的重開了」寫在 `docs/runbooks/settings-page.md`。
