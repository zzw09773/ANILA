...
| `ANILA_ALLOW_DEV_SECRET` | CSP, ingestion-worker | 設 `1` 時 `startup_security` 對 dev 預設值僅 warn；正式環境必拿掉 (改 `0` 或 unset) |
| `ENABLE_CARD_LOGIN` | CSP | branch SSO:是否註冊 `/api/auth/card/*` endpoints。內網 prod 必開 (`true`) |
| `REQUIRE_CARD_LOGIN_ONLY` | CSP | branch SSO:內網 prod 必開 (`true`)。一旦啟用,本機帳密 / OIDC / 自助註冊 endpoints 全回 404,唯一登入路徑是中科院 PKI 卡。`startup_security` 強制 `ENABLE_CARD_LOGIN=true` 同時也要 `true`,否則 fail-fast 拒絕啟動 (避免 bricked 無人能登入) |
| `CARD_INITIAL_OWNERS` | CSP | branch SSO:CSV 員工編號清單。列在裡面的人第一次刷卡建為 `role="owner"` + `is_approved=True` (bootstrap)。其他人走 pending → 完成註冊 → admin 核准流程 |
| `VITE_CARD_COMPONENT_ORIGIN` | UI build | branch SSO:中華電信 HiPKI 本機元件 origin。預設 `http://localhost:16888`,只有特殊內網部署才需要覆寫 |
...
| `SMOKE_USER_API_KEY` | CSP | 自動 seed 的 smoke 使用者 API Key (僅 dev;prod 已從 `AUTO_SEED_API_KEYS` 移除) |
...
ANILA 對齊 ISO/IEC 42001:2023(AI Management System)的治理文件集中在 [`docs/governance/`](./docs/governance/),由 [`iso-42001-compliance.md`](./docs/governance/iso-42001-compliance.md) 作為主索引,盤點 Clause 4–10 與 Annex A 控制的現況、差距、收斂計畫(Phase 1 / 2 / 3)。
...
| [`risk-register.md`](./docs/governance/risk-register.md) | 6.1.2 / 8.3 | 風險登錄簿(15 條 active) |
| [`aiia-template.md`](./docs/governance/aiia-template.md) | 6.1.4 / A.5.2 | AI Impact Assessment **填寫範本** — 每個 production agent 必填 |
| [`model-card-template.md`](./docs/governance/model-card-template.md) | A.6.2.7 | Model card **填寫範本** — 每個 production model 必填 |
| [`data-governance.md`](./docs/governance/data-governance.md) | A.7.x | 資料治理:分類、來源、品質、生命週期 |
| [`ai-incident-response.md`](./docs/governance/ai-incident-response.md) | 10.1 / A.8.3 | AI 事件 4-tier 分級 + 回應流程 |
| [`third-party-ai-register.md`](./docs/governance/third-party-ai-register.md) | A.10 | 第三方 AI 供應商登錄 + 評估流程 |

**Schema 對應**:`agents.{source_commit_sha, last_reviewer_id, vv_status, aiia_doc_path}` + `model_registry.{model_card_url, training_dataset_ref, weights_sha256, intended_use, limitations}` 於 migration `0035_iso_42001_traceability.py` 引入。

**UI disclosure**:Composer 下方 footer 標示「AI 系統 · 內容僅供參考」,hover 顯示完整 AI policy 連結(對應 A.8.2)。
...
- **Backend 認證流程** ([myCSPPlatform/backend/app/services/card_auth.py](./myCSPPlatform/backend/app/services/card_auth.py)):純函式 `verify_pkcs7_signature(signature_b64, card_serial=None)` 解 PKCS#7 → 挑 end-entity signer cert → 抽 `subject.serialNumber` (員工編號) / `subject.commonName` (姓名) / `SAN.rfc822Name` (email) 成 `CardClaims`。Service 層 [card_auth_service.py](./myCSPPlatform/backend/app/services/card_auth_service.py) 包 challenge JWT (一次性 nonce, aud=card-challenge, 2 分鐘 TTL) + registration JWT (給 pending 使用者填 department 用,aud=card-registration, 15 分鐘 TTL) + get-or-create-user (`CARD_INITIAL_OWNERS` 列表內 → owner+approved 直接登入;其他人 → pending 走 admin 核准)。
- **Pending registration / approval flow**:首次刷卡 + 不在 `CARD_INITIAL_OWNERS` → backend 回 HTTP 202 帶 `registration_token`,前端跳「完成註冊」表單 (從 `Department` 表選 active 單位)。送出後狀態切到 `pending_approval`,等 admin 在 `/users` 點 approve 後下次刷卡才能拿 cookie session。`Department` 表跟 `/departments` admin UI 為此實作搭配。
- **前端 caAuth.js** ([myCSPPlatform/frontend/src/api/caAuth.js](./myCSPPlatform/frontend/src/api/caAuth.js)):五個 primitive — `detectCard()` / `generateTbs()` / `signWithCard()` / `submitSignature()` / `loginWithCard()` 共用一個 `runPopupRoundTrip()` popup 生命週期 helper。對齊 [cht/templates/popupForm.html](./cht/templates/popupForm.html) 的 postMessage 協議:主站 open popup 到 `localhost:16888/popupForm` → popup 發 `{func: "getTbs"}` → 主站回 `GetUserCert` 或 `MakeSignature` payload → popup 在自己的 origin fetch `/cht_api/{pkcs11info,sign}` → postMessage result。LoginView UX 改成 detect-first:先按「detect card」看到自己的姓名 + 員工編號 + cardSN,確認後再輸 PIN + sign & submit。
- **Prod 內網模式切換**:`.env` / [docker-compose.yml](./docker-compose.yml) / [myCSPPlatform/docker/nginx.conf](./myCSPPlatform/docker/nginx.conf) 一輪硬化 — secret 預設值改 `${VAR:?must be set}` (缺 `.env` 直接 fail-fast),`ANILA_ALLOW_DEV_SECRET` 預設改 `0`,`REQUIRE_CARD_LOGIN_ONLY` / `ENABLE_CARD_LOGIN` 預設 `true`。Cold-storage services (codeserver / n8n / gitlab) 在 nginx 兩個 server block (port 443 + 4443) 加 `return 404` stealth 鎖死,要解凍時拿掉那行即可。
- **內網部署腳本** ([scripts/build-and-export-for-intranet.sh](./scripts/build-and-export-for-intranet.sh)):外網一鍵 build + pull + save 全部 image 成 `.tar.gz`(自家 6 個 + base 3 個 + cold 3 個 + models 4 個,共 16 個 image) + 產 sha256 manifest + 產內網端 `INTRANET-LOAD.sh` 一鍵 import script。內網無外網連線完全可離線部署。
- **STRICT mode 程式清理**:`_verify_strict()` / `VerifyMode` enum / `_load_ca_bundle()` / `card_revocation.py` (OCSP/CRL) / `extract_ncsist_ca_bundle.py` / `test_card_auth_strict.py` / `docs/card-login-strict-mode-runbook.md` / `backend/data/ncsist-ca-bundle.pem` 全刪;`config.py` 砍掉 `CARD_VERIFY_MODE` / `NCSIST_CA_BUNDLE_PATH` / `CARD_CHECK_REVOCATION`;`requirements.txt` 拿掉 `asn1crypto`。`card_auth.py` 從 477 行 → 175 行;`card_auth_service.py` 拿掉 mode dispatch + revocation helper。AUTO_SEED_API_KEYS 也清掉 smoke-user entry,只剩 ingestion-worker。
- **Dead code 清理**:`ANILA_UI/anila-ui/src/login.jsx` + `src/runtime/card-login.js` 整檔刪除 (nginx `/login` 永遠 redirect 到 myCSPPlatform Vue,React 那邊路由走不到)。`runtime/auth.jsx` 拿掉 `login()` / `loginWithCard()` / `providers` state,只剩 `user` / `isAuthenticated` / `logout` 給 ProtectedRoute 用。`api/cardLogin.js` 被 `caAuth.js` 取代後刪除。
- **測試**:14 個跟 SSO 重構直接相關的 test 全綠 (11 個 card_auth 純函式 + 3 個 startup_security lockdown assertion)。
- 對應 commits:待整理 (建議拆三個 — prod env 切換 + STRICT 清理 + caAuth.js refactor)。
