# ANILA 平台 — `dev-public`(對外網開發 / 測試版)

> **Runtime-first、On-prem 多 Agent 平台。** 三個核心服務、一個落地 LLM，docker compose 一鍵啟動。

> 🧪 **你正在看 `dev-public` 分支**(對外網 dev，純帳密)。= `main` + dev tooling（code-server / n8n / GitLab）+ 寬鬆 hardening，給對外網功能的開發 / 測試前置使用。
>
> | Branch | 部署對象 | 認證 | 定位 |
> |---|---|---|---|
...
> | **`dev-public`** ← *你在這* | 對外網 dev | 純帳密 | main + dev tooling |
> | `dev-military` | 國軍 dev | 純帳密 | dev-public + military 客製 |
> | `trial-military` | 國軍 trial / 展示 | 純帳密 | main 精簡子集 |
>
> **`main` 是 SSOT** — 本分支緊貼 main，差異主要在 dev tooling 的啟用與寬鬆設定。`[dev-only]` 標籤的 commit 只進 `dev-public` / `dev-military` 兩條 dev branch。完整 SOP 見 [`docs/branch-sync-backlog.md`](./docs/branch-sync-backlog.md)。
...
|---|---|
| **用途** | 對外網功能的開發 / 整合測試前置 |
| **認證** | 純帳密（本地登入）；可開 `ANILA_ALLOW_DEV_SECRET=1` 用 dev 預設值快速起 |
| **dev tooling** | code-server（`/codeserver`）/ n8n（`/n8n`）/ GitLab（`/gitlab`）可解凍啟用，方便 agent 開發、工作流測試、本地 Git |
| **hardening** | 寬鬆（dev 取向）；正式對外請改用 `prod-public-passwd` |
| **與 main 的關係** | 緊貼 main；目前內容與 main 對齊，dev tooling 以解凍 / 寬鬆設定的方式區分 |

> ⚠️ **這是 dev 分支，不要直接拿去對外正式服務。** 對外正式環境請用 `prod-public-passwd`（已移除 dev tooling + 外網 hardening）。
...
| [`ANILA_UI/anila-ui`](./ANILA_UI/anila-ui/) | **Chat Runtime UI** — React（dev 直跑 `:5173`） | nginx 前 |
| [`ANILALM`](./ANILALM/) | **Knowledge-base + Studio SPA** | nginx 前 |
| **`nginx`** | 對外閘道；6 個安全 header；dev 可掛 `/codeserver`、`/n8n`、`/gitlab` | `:80` / `:443` / `:4443` |
| **`redis`** | ingestion-worker queue + token-revoke pub/sub | （無 host port） |
| **dev tooling**（解凍時）| code-server / n8n / GitLab — agent 開發、工作流自動化、本地 Git | 經 nginx 子路徑 |
...
flowchart TB
    devs["🧑‍💻 開發者 / 測試者"]
    nginx["nginx :80 / :443<br/>6 安全 header · dev 子路徑"]
    ...
    subgraph devtools["dev tooling（解凍時）"]
    ...
    devs -->|cookie / sk-*| nginx --> anila_ui & anilalm & csp_ctrl & csp_data & router_core & codeserver & n8n & gitlab
...
```bash
cp .env.example .env       # dev 可設 ANILA_ALLOW_DEV_SECRET=1 用預設值快速起
docker compose up -d
```

dev 預設值（`SECRET_KEY=dev-secret-key-change-in-prod` / `ADMIN_PASSWORD=changeme` 等）在 `ANILA_ALLOW_DEV_SECRET=1` 時只 warn 不擋。把 `LOCAL_LLM_BASE_URL` 指向你的測試 LLM（Ollama / vLLM）。

### 解凍 dev tooling

要啟用 code-server / n8n / GitLab：取消 `docker-compose.yml` 對應服務區段的註解，參考區段上方註解設好必要 env（如 `CODESERVER_PASSWORD` / `CODESERVER_WORKSPACE` 必須指**非機密目錄**），再 `docker compose up -d <service>`。nginx 對應子路徑路由本分支已備妥。

> code-server hardening（不要回退）：用 `codercom/code-server` 上游（非 lscr.io/linuxserver fork）、workspace 勿掛 repo root（避免暴露 `.env` / 私鑰）。n8n：`NODE_TLS_REJECT_UNAUTHORIZED` 預設 `1`、Code Node 模組白名單寫死。
...
| `ANILA_ALLOW_DEV_SECRET` | CSP / Worker | dev 設 `1` 時對 dev 預設值僅 warn（**勿用於對外正式**） |
| `CSP_SECRET_KEY` / `CSP_SERVICE_TOKEN` | CSP / Agent / Router | JWT 主鑰 / s2s token（dev 可用預設） |
...
| `CODESERVER_PASSWORD` / `CODESERVER_WORKSPACE` | code-server | 解凍 code-server 時必設；workspace 須指非機密目錄 |
...
- **dev 寬鬆**：`ANILA_ALLOW_DEV_SECRET=1` 時 dev 預設值不擋。**對外正式請用 `prod-public-passwd`**（移除 dev tooling + 外網 hardening）。
...
- dev tooling 暴露面較大（code-server / n8n / GitLab），**僅限受信任的 dev 網段**使用。

---

## 分支與同步

- 本分支 = `main` + dev tooling + 寬鬆 hardening。
- 從 main sync：`git merge origin/main -X theirs`。
- `[dev-only]` 標籤的 commit 只進 `dev-public` / `dev-military` 兩條 dev branch。
- `dev-military` 以本分支為基礎再加 military 客製。
...
**分支**：`dev-public`（對外網 dev / 純帳密）· **部署**：`docker compose up -d`（dev）· **對外正式請用**：`prod-public-passwd` · **分支同步**：[`docs/branch-sync-backlog.md`](./docs/branch-sync-backlog.md)
