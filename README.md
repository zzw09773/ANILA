...
2. 把 service name 加進 [`docker-compose.yml`](./docker-compose.yml) `csp.environment.ANILA_TRUSTED_HOSTS`（comma-separated）
3. `docker compose up -d csp`（讓 CSP 吃新的 trusted hosts env）
4. `docker compose -f models/docker-compose.yml up -d <service-name>`
5. CSP UI `/models` → Add Model，endpoint = `http://<service-name>:<port>/v1`，`internal` checkbox 預設打勾保留
...
- **SSRF guard**: 模型 endpoint URL 註冊現在會被 [`anila_core.security.validate_outbound_url()`](./anila-core/src/anila_core/security/url_guard.py) 守門。docker service name 屬 single-label hostname 預設被擋，靠 `ANILA_TRUSTED_HOSTS` 顯式 allow-list 放行。**新增模型 service name 後一定要把它加進 env，不然 CSP UI 註冊會 400**。
...
- **GPU 排程**: 用 `device_ids: ["N"]` 顯式綁卡，一卡一 service。要 replica scale 在 compose 加新 service entry 指到不同 `device_ids`。
...
| `ANILA_TRUSTED_HOSTS` | CSP / Worker / Agent | SSRF guard allow-list (comma-separated)。docker 內部 service names 預設被 guard 擋,列在這條 env 才放行。新增模型 service 後一定要加進來 + `docker compose up -d csp` 重啟 |
...
- **SSRF guard**：`anila_core.security.url_guard.validate_outbound_url` 集中 deny-list（loopback / private / link-local / cloud-metadata / docker service name / `*.internal` / `*.local` 等），對 user-supplied `endpoint_url` 一律驗證。Agent register / update + 使用者 LLM credential + **model registry** 都接此 guard;agent endpoint 變更時 `approval_status` 自動退回 `pending` 強制 admin 重新核可。`ANILA_TRUSTED_HOSTS` env (comma-separated) 是顯式 allow-list,管理員列上的 docker service name 才能通過 single-label / internal-zone 阻擋。
...
- **SSRF guard parity** ([anila-core/src/anila_core/security/url_guard.py](./anila-core/src/anila_core/security/url_guard.py)): 新增 `ANILA_TRUSTED_HOSTS` env-driven allow-list,以「scheme 校驗後、所有 host 校驗前」short-circuit 放行 admin-blessed docker service names。`agents.py` / `credentials.py` 已有 guard,**model_registry 之前完全沒守 — 一併補上 parity**(`api/models.py::_enforce_endpoint_url` 在 create / update 路徑都呼叫)。新加 url_guard / models SSRF / is_internal 共 3 個測試檔。
...
- **CSP env 配置** ([docker-compose.yml](./docker-compose.yml)): csp service 加 `anila-models-net` 進 `networks:`、env 加 `ANILA_TRUSTED_HOSTS` 預設值 `gpt-oss-20b,gemma4,nv-embed-proxy,host.docker.internal`(最後一個保留 backward compat 給既有 7011 embedding-proxy 註冊);順手把 csp / router 拿掉 `ports:`(改 `expose:`),外部入口只剩 nginx `:443` 一條 HTTPS + API key。`extra_hosts: host.docker.internal:host-gateway` 既有,不動。
- **Cutover 流程**: 1) `docker network create anila-models-net` 2) `docker compose up -d csp` 3) 停舊 stack (`docker compose -f /home/aia/c1147259/project/My-OpenAI-Frontend/docker-compose.migration.yml down`) 4) `docker compose -f models/docker-compose.yml up -d` 5) CSP `/models` UI 把 3 條 endpoint 從 LAN IP 改成 docker service DNS + 勾 internal。
...
**Last updated**: 2026-05-11（推論模型 stack 解耦 + SSRF guard parity + is_internal）· **Maintainers**: ANILA 平台團隊 · **Single source of truth**: [`anila_plan.md`](./anila_plan.md) · **記憶層設計**：[`docs/briefing/anila-memory-layer-rfc.md`](./docs/briefing/anila-memory-layer-rfc.md) · **資安／SSO 規劃**：[`docs/sso-migration.md`](./docs/sso-migration.md)、[`docs/sprint-7x-plan.md`](./docs/sprint-7x-plan.md)、[`docs/runbooks/rotate-tls-cert.md`](./docs/runbooks/rotate-tls-cert.md) · **Service-token cutover**：[`docs/runbooks/service-token-cutover.md`](./docs/runbooks/service-token-cutover.md) · [`docs/runbooks/legacy-agent-bootstrap.md`](./docs/runbooks/legacy-agent-bootstrap.md)
