...
| 子專案 | 角色 | 預設 Port |
|---|---|---|
| [`myCSPPlatform`](./myCSPPlatform/) | **CSP**（Control & Data Plane）— 使用者 / API Key / 模型 / 對話 / 附件 / 分享 / 交接 / 審計 / OpenAI 相容代理 / Studio / Ingestion / Evaluator | `:8000` |
| [`anila-core`](./anila-core/) | **Runtime foundation（SDK）** — Python agent runtime 基座（api / registry / engine / tools / providers / storage / memory / compact / cli / **security**）。Router 與所有 agent 共用 | — |
| [`anila-core-router`](./anila-core-router/) | **Router** — OpenAI 相容分派器；依請求自動路由到註冊的 Agent；Sprint 8 X 起接 `service_clients` per-credential token | `:9000` |
| [`ANILA_UI/anila-ui`](./ANILA_UI/anila-ui/) | **Runtime UI** — React 聊天介面，串 CSP（cookie + SSE）與 Router（`anila-router` pseudo-agent）。經由 nginx 對外，dev 跑 `:5173` | nginx 前 |
...
| **`nginx`**（compose service）| 對外閘道；同源 reverse-proxy `/api`、`/v1`、`/anilalm/`、`/static`、`/uploads`，前端配 nonce-CSP / HSTS / Permissions-Policy 等 6 個安全 header | `:80` / `:443` / `:4443` |
| [`runtime_logic`](./runtime_logic/) | **TS Runtime 參考材料**（READ-ONLY）— 用來對照移植到 `anila-core` 的 agent runtime 設計原本；原始碼 gitignored | — |

> Sprint 8 X 開放後仍漏列在子專案表（待 Sprint 9 X README 大整修）：`ingestion-worker`（Arq + Redis async pipeline）、`ANILALM` + `pptx-renderer`（KB 前端 + PPTX 服務）、`redis`（ingestion-worker queue）。詳見 [`docker-compose.yml`](./docker-compose.yml)。
...
啟動順序（由 healthcheck 串接）：`csp-db` → `csp` + `redis` → `ingestion-worker` → `router` → `anila-ui` + `anilalm` + `pptx-renderer` → `nginx`（最後對外閘道）。首次啟動約 30 秒（含 alembic migration 至 `0027` + 自動 seed smoke 使用者與 API Key）。
