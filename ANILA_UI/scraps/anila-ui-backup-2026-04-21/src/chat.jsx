// Chat view — main ANILA runtime (trust + multi-agent + collab enabled)

const {
  useState: _uS, useEffect: _uE, useRef: _uR, useMemo: _uM, useCallback: _uC,
} = React;

// ---- Trace Row + Routing Trace (kept from v1) ----
const TraceRow = ({ event, active, done }) => (
  <div style={{
...
const RoutingTrace = ({ trace, stage, routedAgent, done }) => {
  const [open, setOpen] = _uS(false);
  const traceStyle = window.ANILA_TWEAKS?.traceStyle || "collapsible";
...
// ---- Message Bubble (now with citations, confidence, audit, follow-ups, redaction, classified corner) ----
const MessageBubble = ({ msg, agents, conversationId, classified, onRegenerate, onOpenCitation, onPickFollowUp }) => {
...
const AgentSelector = ({ agents, value, onChange }) => {
  ...
  return (
    <Dropdown align="left" width={360} trigger={(open) => (
      <button style={{
        ...
        <span style={{ fontSize: 11, color: "var(--fg-subtle)", fontFamily: "var(--font-mono)" }}>
          {selected.id === "anila-router" ? "auto" : selected.short}
        </span>
      ...
      {(close) => (
        <div>
          ...
          {agents.map(a => (
            <MenuItem key={a.id} active={a.id === value}
...
// ---- Composer (PII hint + @-mention highlight) ----
const Composer = ({ onSend, disabled, agents, redactionMode = "mask", initialValue = "" }) => {
  const [text, setText] = _uS(initialValue);
  const [atts, setAtts] = _uS([]);
  const taRef = _uR(null);
  const [mode, setMode] = _uS(redactionMode);

  const piiHits = _uM(() => detectPII(text), [text]);
  const mentionParse = _uM(() => parseMentions(text, agents || []), [text, agents]);
  ...
  return (
    <div style={{
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: "8px 10px 0" }}>
          {atts.map((a, i) => (
            <div key={i} style={{
              ...
              <IconButton style={{ width: 18, height: 18 }} onClick={() => setAtts(list => list.filter((_, j) => j !== i))}>
                <IconX size={11}/>
              </IconButton>
            </div>
          ))}
      ...
      <textarea
        ...
        onKeyDown={onKey}
        placeholder="問 ANILA 任何事情 — 用 @agent 指定 agent · Shift+Enter 換行"
        rows={1}
        style={{
          width: "100%",
      ...
      <div style={{
        ...
        <button onClick={submit} disabled={disabled || (!text.trim() && atts.length === 0)}
          style={{
...
// ---- Sidebar with folder tabs ----
const Sidebar = ({ conversations, selectedConvId, onSelectConv, onNewChat,
                   agents, onOpenAgentBrowser, user, onLogout, onOpenSettings,
                   collapsed, onToggleCollapsed,
                   folder, setFolder, folders, onOpenTagEditor }) => {
  const [tab, setTab] = _uS("chats");
  ...
  const filtered = conversations.filter(c => {
    ...
    // query supports tag:xxx syntax
    const q = query.trim().toLowerCase();
    if (!q) return true;
    const tagMatch = q.match(/^tag:(\S+)(?:\s+(.*))?$/);
    if (tagMatch) {
      const tag = tagMatch[1], rest = (tagMatch[2] || "").trim();
        ...
        <>
          ...
          <div style={{ padding: "0 10px 6px" }}>
            <div style={{
              ...
              <input value={query} onChange={e => setQuery(e.target.value)} placeholder="搜尋… (tag:hr 特休)"
                ...
                <div key={c.id} style={{ position: "relative" }}>
                  ...
                  onMouseLeave={e => { if (c.id !== selectedConvId) e.currentTarget.style.background = "transparent"; }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 3, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 11, color: "var(--fg-subtle)" }}>{c.ts}</span>
                  ...
                  <div style={{ position: "absolute", right: 4, top: 5 }}>
                    <Dropdown align="right" width={260} trigger={() => (
                      ...
                      {(close) => <TagEditor folders={folders} conversation={c}
                        onUpdate={(patch) => onOpenTagEditor(c.id, patch)} close={close}/>}
                    </Dropdown>
                  </div>
                </div>
              );
          ...
          {agents.map(a => (
            <div key={a.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 500 }}>{a.name}</div>
                <div style={{ flex: 1 }}/>
                <span style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--fg-subtle)" }}>{a.short}</span>
              </div>
...
Object.assign(window, { MessageBubble, AgentSelector, Composer, Sidebar, RoutingTrace });
