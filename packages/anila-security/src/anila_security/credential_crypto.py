...
Originally centralised from the CSP backend so both CSP (encrypt at
create-time) and ingestion-worker (decrypt at judge-call-time) share one
implementation. Gate 1 F4 moved the implementation into the standalone
``anila-security`` package so consumers no longer inherit anila-core's
runtime, database, and web-framework dependencies.
