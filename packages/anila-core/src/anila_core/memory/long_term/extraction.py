...
# 儲存層一個 (user_id, key) 只能一列。籠統 key 重複使用會在寫入時整批失敗。
# value 必須是使用者原話的連續片段：證據檢查對整段值做子字串比對，
# 用分號把分開的句子拼起來會整筆被丟掉。
_FACT_KEY_RULES = (
    "每個事實一個 key、一個 value。每個 key 只出現一次。\n"
    "同一類別有多個值時，寫進同一個 value，不要重複 key。\n"
    "value 必須是使用者原話裡連續出現的片段，零件名稱、尺寸、數字照原話保留，"
    "不要改寫，也不要把分開的句子用分號拼起來。\n"
    "key 要具體。請優先使用：單位、職責、專案技術選擇、長期偏好。\n"
    "使用者親口說的專案決定收成「專案技術選擇」。助理說的話不是事實。\n"
    "對不上這些類別時，另給一個具體 key，仍然每個 key 只出現一次。"
)


EXTRACTION_SYSTEM_PROMPT = f"""你是事實萃取器。只從使用者親口說的話抽出穩定事實
（範例類別：姓名、職稱、長期偏好、長期目標、工作範圍、專案技術選擇）。
...
# 活體上模型把摘要寫成助理的建議，使用者自己的零件、尺寸、數字消失。
# 順序把目標與決定放在助理之前；事實仍只收使用者原話。
MEMORY_REFRESH_SYSTEM_PROMPT = f"""你是記憶整理器。讀完對話後只輸出一個 JSON 物件，不要前言。

summary 依下列順序寫，全文最多 600 字：
1. 使用者的目標。
2. 使用者自己說的決定與限制。零件名稱、尺寸、數字等具體值照原話保留。
3. 未解問題或下一步。沒有就略過。
