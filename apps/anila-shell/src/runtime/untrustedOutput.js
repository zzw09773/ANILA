// 模型輸出裡的外連與 Markdown 圖片。非平台網域改成純文字，避免點擊或載入。
...
function neutralizeSegment(text) {
...
function escapeHtmlOpen(text) {
  return text.replace(/</g, (match, offset, whole) => {
...
/** 程式碼圍欄裡的網址留給預覽，其餘外連改成不能點的文字。原始 HTML 不解析。 */
export function neutralizeUntrustedMarkdown(text) {
