...
| **Four-level classification** | `src/trust.jsx`, `src/runtime/classified.js`, `src/runtime/classifyRetryQueue.js` | `watermarkLevel()` derives the real zh-TW level; `ClassificationWatermark` is a **real four-level corner badge** (密=warn / 機密=danger-strong; 無機密/營業秘密 skip the full watermark), replacing the decorative English "CONFIDENTIAL"; `ConfidentialWatermark` is a **full-screen forensic watermark** tiling "level · user email · trace_id" for screenshot-leak tracing. The one-way latch is backend-driven; `classifyRetryQueue` guarantees the latch reaches CSP. |
