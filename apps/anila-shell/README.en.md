...
| **Five-level classification** | `src/trust.jsx`, `src/runtime/classified.js`, `src/runtime/classifyRetryQueue.js` | `watermarkLevel()` derives the real zh-TW level; `ClassificationWatermark` is a **real five-level corner badge** (機密=warn / 極機密=danger / 絕對機密=danger-strong), replacing the decorative English "CONFIDENTIAL"; `ConfidentialWatermark` is a **full-screen forensic watermark** tiling "level · user email · trace_id" for screenshot-leak tracing. The one-way latch is backend-driven; `classifyRetryQueue` guarantees the latch reaches CSP. |
...
| Area | Detail |
|---|---|
| Framework / router | **React 18.3.1** · `react-router-dom` 6.30.1 |
