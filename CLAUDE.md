...
- 測試——🔴 **判準是規矩,不是數字**:**三個套件都必須被跑**(csp、anila-shell、
  ...
  **跑 csp 全套的正確指令(照抄,不要憑印象)**:
  ...
  csp **2627/73/0**(2026-08-20 Reviewer,764s;前次 08-17 稽核長 2572/73/0、08-11 2538/73
  ——漂移量＝其間合併的新測試,**這正是「數字用來偵測漂移」的設計在運作**)、
  anila-shell **366**(08-11)、治理中心 **144/143/1**(08-17,`npm ci` 後 `npm test`)。
...
- **待裁決集中在 `docs/OWNER-QUESTIONS.md`**(24 題,**不要重問**;⚠ 數字會過期,以檔內標記為準)。
