...
的逐位元組副本(vendor 於 2026-07-17,來源 md5 a72c6a615eacf9e18e419b2300c7cb0a)。

**改這裡就必須同步改 studio 那份,反之亦然。** 這是 JWT 信任錨的一部分:
兩份分歧 = 被撤銷的權杖在一個服務被擋、在另一個服務仍然通行。

為什麼是副本而不是共用套件:見 `jwks_client.py` 同一段標頭與規劃書 §12。

vendor 時只加了這段標頭,其餘一行未改。隱性相依是 `app.config.settings` 的
同名欄位(`CSP_BASE_URL`、`CSP_SERVICE_TOKEN`、`REDIS_URL`、
`REDIS_REVOCATION_CHANNEL`、`REVOCATION_CACHE_TTL_SECONDS`、
`REVOCATION_RECONCILE_INTERVAL_SECONDS`、`INTERNAL_TIMEOUT_*`)。

⚠ fail-closed 姿態一併繼承:cache 沒 ready(Redis 斷線 / 冷啟動未完成)時
一律拒絕認證。對 gateway 而言就是 **Redis 或 csp 一倒,ASR 全斷**(WS 一律
被拒)。這是刻意的,operator 排查表必須把這兩個上游列在最前面(規劃書 §2.3)。
