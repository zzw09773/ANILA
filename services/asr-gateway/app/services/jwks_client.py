...
的逐位元組副本(vendor 於 2026-07-17,來源 md5 afe6eb4113bad8587abe3c68da59dc30)。

**改這裡就必須同步改 studio 那份,反之亦然。** 這是 JWT 信任錨的一部分:
兩份分歧 = 兩個服務對「什麼是有效權杖」有不同認定,而安全修補只補到一邊。

為什麼是副本而不是共用套件:抽到 `packages/anila-core` 才是正解,但那要改
anila-studio —— 一個已部署、鎖 image content ID、橫跨七條分支的服務,明顯
超出「新增語音輸入」的範圍。這筆技術債記在
`docs/planning/asr-voice-input-plan.md` §12,由專案自行排期償還;**不要在
ASR 的 PR 裡順手做跨服務重構**。

vendor 時只加了這段標頭,其餘一行未改。唯一的隱性相依是 `app.config.settings`
必須提供同名欄位(`CSP_BASE_URL` / `JWKS_REFRESH_SECONDS` /
`INTERNAL_TIMEOUT_CONNECT`)—— gateway 的 config.py 刻意與 studio 對齊,
就是為了讓這份副本能原樣運作。
