...
def test_hard_delete_keeps_audit_attribution(client: TestClient, db):
    ...
    # Audit row 還在，而且 actor 的數字身分沒有被洗掉（P2.7 起的行為）。
    # ⚠ `expire_all()` 不是裝飾:conftest 的 session 是 ``expire_on_commit=False``,
    # identity map 裡那顆 AuditLog 還握著刪除前讀到的值。不 expire 就等於在斷言
    # 記憶體裡的舊值——production 到底有沒有動那一欄,這個測試會看不出來。
    db.expire_all()
