...
def test_there_is_still_no_env_file_shortcut():
    ...
    # 找的是 ``env_file:`` **這個鍵**,不是這七個字元。兩次踩過的坑:
    #   1. 2026-08-07 有一包在 platform.yml 加了一句**說明「這裡沒有
    #      env_file」的註解**,原本的字串搜尋把那句註解判成違規 ——
    #      守衛對著自己要保護的那句話轉紅。
    #   2. 改用 ``yaml.safe_load`` 解析也不行:overlay 檔用了 ``!reset``
    #      這種 compose 自己的標籤,safe_load 直接丟 ConstructorError,
    #      於是守衛變成「解析器不認得就當壞掉」。
    # 逐行去掉註解再比對鍵,對兩者都免疫。
