...
_REGEX = tuple(
    ...
    for p in (
        ...
        # agent 領域拒答（2026-09-02 活體：「本系統僅提供法律文件檢索…無法回答與法律無關的問題」）：
        # 「僅提供／僅處理／僅支援 … 無法回答」在開場短窗內才算，grounding 的
        # 「段落沒有提供…無法回答」不含「僅提供…服務」不會命中。
        rf"僅(?:提供|處理|支援|限).{{0,30}}{_ZH_REFUSAL_VERB}.{{0,12}}回答",
    )
)
