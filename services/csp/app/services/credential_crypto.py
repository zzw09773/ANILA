...
``anila_security.credential_crypto`` so the ingestion-worker can
import it without depending on the CSP backend's package layout
(worker container has anila-security but not the CSP backend code).
...
unchanged. New code should import from anila-security directly:

    from anila_security import encrypt_credential, decrypt_credential
"""
