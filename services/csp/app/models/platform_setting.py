...
可調的東西一律是環境變數，而環境變數要改就得重啟容器，管理員自己動不了；本輪
只把十二顆真正的即時 C 類控制保留在這張表。

⚠ **這張表是「設定頁」那件工程的地基，所以它的第一條不變式是「讀不快取」。**
畫面上改了一個數字、後端還在讀舊值，是本平台可能出的最大的假控制項。因此
本模組的 getter 一律是一次主鍵查詢，**不做**模組層快取、不用 ``lru_cache``、
不在啟動時預讀。其餘部署、秘密與程式常數不再假裝是這張表的控制項；在這裡放
一個「只是為了效能」的快取，仍會把真正的即時控制變成假控制項。
...
def _coerce_bool_for_write(spec, raw: str) -> bool:
    ...
    讀取端的規則**不動**（env 的判準是消費模組的既成事實，改了就等於騙人）；
...
def set_setting(db: Session, key: str, value: Any, *, actor: User | None = None) -> None:
    ...
    目前登錄表只有 C 類；類別閘門仍保留在這一層，避免端點手抄可寫名單。
