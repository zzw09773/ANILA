...
Why this exists
---------------
Many ORM columns were ``timestamp without time zone`` and come back as
...
stays correct after ``timestamptz`` column conversion and can remain
permanently.

Nested response models that also inherit this base apply the same rule.
Nested foreign models that already require ``AwareDatetime`` keep emitting
...
Reclaimed from attic ``fe9fc59`` (ApiResponseModel + as_utc serializer);
rewritten only for docstring/path fit on the restart tree.
"""
