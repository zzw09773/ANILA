...
Studio-side copy of the CSP helper tests. Each service's suite must go
red on its own tree: adding an un-neutralized producer under
``services/anila-studio/app`` has to fail here, not only in CSP.
...
from app.services.csv_formula import FORMULA_TRIGGER_PREFIXES, csv_formula_safe


_SERVICE_ROOT = Path(__file__).resolve().parents[1]
_APP_ROOT = _SERVICE_ROOT / "app"
_TWIN = _SERVICE_ROOT.parent / "csp" / "app" / "utils" / "csv_formula.py"
...
def test_twin_implementations_have_identical_logic():
    local = _APP_ROOT / "services" / "csv_formula.py"
...
def test_twin_docstrings_name_the_other_copy():
    local = (_APP_ROOT / "services" / "csv_formula.py").read_text(encoding="utf-8")
    twin = _TWIN.read_text(encoding="utf-8")
    assert "services/csp/app/utils/csv_formula.py:26" in local
...
def test_every_studio_spreadsheet_producer_is_neutralized_or_closed_domain():
    offenders = [
        p
...
def test_scan_finds_exporter_without_a_file_list():
    hits = {p.name for p in iter_spreadsheet_producers(_APP_ROOT)}
    assert "datatable_exporter.py" in hits
...
def test_guard_goes_red_for_a_second_bare_writer_in_a_file_that_imports_the_helper(
    ...
    """Reviewer scenario: a file that already imports the helper; a second
    function with a bare csv.writer / Workbook must still fail the guard."""
    evil = tmp_path / "exporter_like.py"
    evil.write_text(
        "import csv, io\n"
        ...
        "def to_csv(rows):\n"
        "    w = csv.writer(io.StringIO())\n"
        "    w.writerow([csv_formula_safe(x) for x in rows])\n"
        "\n"
        "def export_raw_v2():\n"
        "    w = csv.writer(io.StringIO())\n"
        "    w.writerow(['=1+1'])\n",
