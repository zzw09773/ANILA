"""RAG Pre-processor for the ANILA Core query engine.
...
Injection format:
    ...
    --- Source 1 (score: 0.92, doc: report.pdf) ---
    {chunk_content}
    --- Source 2 (score: 0.87, doc: manual.docx) ---
    {chunk_content}
    [End RAG Context]
...
from ..models.storage import DocumentChunk, RetrievalTrace
...
class RagPreprocessor:
    ...
    async def preprocess(
        ...
        try:
            chunks: list[DocumentChunk] = await self._retriever.search(
        ...
        logger.debug(
            "RAG: injected %d chunks (%.1f ms) for query=%.60r",
            len(chunks), latency_ms, query,
        )
...
def _format_context(chunks: list[DocumentChunk]) -> str:
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        lines.append(f"--- Source {i} (score: {score:.2f}, doc: {source}) ---")
